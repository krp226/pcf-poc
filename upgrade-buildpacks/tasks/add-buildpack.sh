...
do
    ...
    if [ -z "${existing_buildpack}" ]; then
      ...
      cf create-buildpack ${AUX_BUILDPACK_NAME}_${VERSION} buildpack/*-$STACK_NAME-*.zip $NEW_POSITION --enable
      ...
      #cf update-buildpack ${AUX_BUILDPACK_NAME}_${VERSION} -p buildpack/*-$STACK_NAME-*.zip -s $STACK_NAME -i $index --enable
    fi
done
cf buildpacks
